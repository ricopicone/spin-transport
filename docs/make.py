...
def html(name, options='--no_abort', split=False):
    """
...
def main():
    ...
    html(dofile, options=common_options + common_html_options + bootstrap_options + ' --no_abort --html_style=bootswatch_journal --html_output=%s' % dofile, split=True)

    ipynb(dofile, options=' --no_abort')

    doconce2format(dofile, format, options=common_options + ' --no_abort')

    # build LaTeX
    latex(
        dofile,
        'pdflatex',
        '--latex_style=std --latex_title_layout=std --latex_font=palatino --latex_admon=yellowicon --latex_admon_color=yellow!5 --latex_fancy_header --latex_code_style=pyg-gray --minted_latex_style=friendly --latex_section_headings=black --latex_colored_table_rows=blue --latex_preamble=latex_preamble.tex --no_abort',
        'screen')
